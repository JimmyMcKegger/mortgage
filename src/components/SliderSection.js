...
const SliderSection = ({ data, setData }) => {
  ...
  const handleAnnualOverPaymentsChange = (value) => {
    setData({
      ...data,
      annualOverPayments: value,
    });
  };
  ...
  return (
    <>
      ...
      />
      <SliderComponent
        label="Annual Over-Payments"
        min={0}
        max={5000}
        step={200}
        defaultValue={data.annualOverPayments}
        value={data.annualOverPayments}
        amount={toCurrency(data.annualOverPayments)}
        onChange={(e, value) => handleAnnualOverPaymentsChange(value)}
      />
    </>
  );
};
